...
module Mangle = struct
  ...
  let make_alias_module t ~has_lib_interface ~src_dir ~interface_name path =
    ...
    let path = if has_lib_interface then [ name ] else interface_name :: path in
    Module.generated ~path ~obj_name ~kind ~src_dir name

  let wrap_module t m ~interface_name ~is_lib_interface =
    let path_for_mangle =
      let path = Module.path m in
      match t with
      | Exe | Melange -> interface_name :: path
      | Lib _ ->
        let path =
...
module Wrapped = struct
  ...
  let find_dep =
    let rec closure_group g =
      ...
      | Alias _ ->
        (* XXX Working around ocamldep's limitations. Hopefully codept will
           make this obsolete *)
    ...
    fun t ~of_ name ->
      ...
      | Wrapped_compat ->
        ...
        if Module_name.equal name (Module.name li) then [ li ] else []
      | _ -> (
        ...
        with
        | None -> []
        | Some node -> closure_node node)

  let group_interfaces (t : t) m =
    Group.parents t.group m
    |> List.filter_map ~f:(fun (g : Group.t) ->
           match Module_name.Map.find g.modules g.name with
           | Some (Module m) -> Some m
           | Some (Group _) | None -> None)
...
let find_dep =
  ...
  let rec find_dep t ~of_ name : Module.t list =
    if Module.name of_ = name then []
      ...
      let result =
        match t with
        | Wrapped w -> Wrapped.find_dep w ~of_ name |> from_impl_or_lib
        | Stdlib s ->
...
let make_singleton m mangle =
  Singleton
    (let main_module_name = (Mangle.prefix mangle).public in
     let m = Module.set_path m [ main_module_name ] in
     Module.set_obj_name m
       (Module_name.Path.wrap [ main_module_name; Module.name m ]))
